...
class SearchTab(QWidget):
    ...
    def populate_results(self, results):
        ...
        if not results:
            return
        for p in results:
            # Remove the first 4 directories from the path
            parts = p.split(os.sep)
            if len(parts) > 4:
                display_text = os.path.join(*parts[4:])  # keep everything after the 4th element
            else:
                display_text = os.path.basename(p)  # fallback
            # Add shortened path to display
            self.results_list.addItem(display_text)
            # Store full path in item data so preview still works
...
class MainWindow(QMainWindow):
    def __init__(self, root_dir):
        ...
        tabs = QTabWidget()
        # Remove blue "Search" text on top by using an empty label
        tabs.addTab(SearchTab(root_dir), "")
        tabs.tabBar().hide()   # Hide the tab bar completely
