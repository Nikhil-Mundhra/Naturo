...
def parse_product_code(code):
    ...
    material = [t for t in tokens_lc if t in [v.lower() for v in MATERIAL_MAP.values()]]
    color = [t for t in tokens_lc if t in [v.lower() for v in COLOR_MAP.values()]]
...
def search_files(root_dir, code):
    ...
    for p in candidate_files:
        ...
        if required_materials and not any(mat in s for mat in required_materials):
            continue
        if required_colors:
            if not any(re.search(r'(^|_)' + re.escape(col) + r'($|_)', s) or re.search(col, s) for col in required_colors):
                continue
...
class SearchTab(QWidget):
    def __init__(self, root_dir):
        ...
        self.design_cb.addItems([""] + list(DESIGN_MAP.keys()))
        ...
        self.top_widget.setMaximumHeight(120)  # Start in code mode height
    ...
    def toggle_mode(self):
        if self.code_radio.isChecked():
            ...
            self.animate_height(120)
        else:
            self.stack.setCurrentIndex(1)
            self.animate_height(300)
    ...
    def do_search_qn(self):
        ...
        # Handle design + count merging
        design_code = ""
        if self.design_cb.currentText():
            design_code = DESIGN_MAP[self.design_cb.currentText()]
        size_text = self.size_input.text().strip()

        if design_code in ("EQ", "L", "S") and size_text:
            # Merge e.g., EQ + 14L → EQ14L
            parts.append(f"{design_code}{size_text}")
            size_text = ""  # already used
        elif design_code:
