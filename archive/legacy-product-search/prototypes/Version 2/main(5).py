...
from PySide6.QtWidgets import (
    ...
    QButtonGroup, QSizePolicy, QToolBox, QScrollArea, QFrame, QCheckBox,
    QDialog, QTextEdit
)
...
# heights (you set them previously)
...
def color_matches(searchable, required_colors):
    # required_colors: list of short codes like 'bl', 'br'
    for col in required_colors:
        # token boundary or direct attachment fallback
...
class SearchTab(QWidget):
    def __init__(self, root_dir):
        ...
        # Collapsible design selector (QToolBox) with Select All / Clear All buttons per category
        self.design_toolbox = QToolBox()
        self.design_checkboxes = {}  # category -> list of QCheckBox
        for cat, designs in DESIGN_CATEGORIES.items():
            ...
            for name in designs.keys():
                ...
                vbox.addWidget(cb)
                cb_list.append(cb)
            # connect buttons (use default args to bind current list)
        ...
        self.size_input.setPlaceholderText('e.g., 12"9.5F or 14L')
        ...
        form.addRow("Design(s):", self.design_toolbox)
    ...
    def do_search_qn(self):
        ...
        # Append designs if any were selected
        if selected_design_codes:
            parts.extend(selected_design_codes)

        # Size (normalize) if any leftover
        if size_text:
        ...
        # Folder and supplier
        #if self.foldercode_input.text().strip():
         #   parts.append(self.foldercode_input.text().strip())
        #if self.supplier_input.text().strip():
        ...
        # Debug logging
        dbg, matches = debug_search_files(self.get_root_dir(), code)
        self.last_debug = dbg
    ...
    def get_root_dir(self):
        # walk up to MainWindow to fetch the root_dir passed there
        parent = self.parent()
        while parent and not isinstance(parent, QMainWindow):
            parent = parent.parent()
        if isinstance(parent, QMainWindow):
            # the MainWindow stored root_dir as attribute
...
class MainWindow(QMainWindow):
    def __init__(self, root_dir):
        ...
        # Never allow the window to be larger than the available screen
        self.setMaximumSize(screen.width(), screen.height())
        # Optionally center within screen bounds
