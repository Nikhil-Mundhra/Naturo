...
class SearchTab(QWidget):
    def __init__(self, root_dir):
        ...
        for cat, designs in DESIGN_CATEGORIES.items():
            ...
            vbox.setContentsMargins(0, 0, 0, 0)   # tighten inner margins
            vbox.setSpacing(2)                    # tighter row spacing

            cb_list = []
            for name in designs.keys():
                cb = QCheckBox(name)
                cb.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)  # lets row be fully clickable
                # make the whole horizontal strip clickable
        ...
        top_cols.setContentsMargins(0, 0, 0, 0)  # remove gap between left & right panes
        top_cols.setSpacing(10)
        top_cols.addWidget(left_widget, 1)
        top_cols.addWidget(right_widget, 2)

        # ===== SEARCH + MASTER CLEAR ALL + ADD TO CART =====
        ...
        search_row.setContentsMargins(0, 0, 0, 0)  # keep buttons snug to questionnaire
        search_row.setSpacing(10)
        search_row.addStretch()
        search_row.addWidget(self.search_btn_qn)
        search_row.addWidget(self.clear_all_btn_master)
        search_row.addWidget(self.add_to_cart_btn)
        search_row.addStretch()
        ...
        qn_main_vbox.addLayout(search_row)
        ...
        controls_row.addWidget(self.clear_all_btn_master)
        ...
        self.reopen_btn.clicked.connect(self.toggle_mode)   # just toggles open/close
        ...
        layout.addLayout(controls_row)          # <-- persistent controls here
    ...
    def do_search_code(self):
        ...
        # Update active filters label for code mode
